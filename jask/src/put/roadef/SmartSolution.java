			...
			if (reassignmentTracker == null) {
				try {
					...
					reassignmentTracker.println(
						...
						"SourceMachineResourceUsage,DestMachineResourceUsage,SourceMachineCapacities," +
						"DestMachineCapacities,SourceMachineTransientUsage,DestMachineTransientUsage," +
						"SourceMachineProcessCount,DestMachineProcessCount,LoadCost,BalanceCost,SolutionCost");
	...
	private void trackProcessReassignment(int processId, int sourceMachine, int destinationMachine) {
		...
		synchronized (trackingLock) {
			...
			for (int r = 0; r < problem.getNumResources(); r++) {
				...
				// Resource usage
				srcUsageBuilder.append(getResourceUsage(sourceMachine, r));
				destUsageBuilder.append(getResourceUsage(destinationMachine, r));
				
				// Machine capacities
				srcCapacityBuilder.append(problem.getMachine(sourceMachine).capacities[r]);
				destCapacityBuilder.append(problem.getMachine(destinationMachine).capacities[r]);
				
				// Transient usage
				srcTransientBuilder.append(getTransientUsage(sourceMachine, r));
				destTransientBuilder.append(getTransientUsage(destinationMachine, r));
			}
			...
			// Count processes in machines
			int srcProcessCount = processesInMachine[sourceMachine].size();
			int destProcessCount = processesInMachine[destinationMachine].size();
			
			// Get costs
			long loadCost = getLoadCost();
			long balanceCost = getBalanceCost();
			long solutionCost = getCost();
			
			// Write tracking data
			reassignmentTracker.printf("%d,%d,%d,%d,%d,%d,%d,\"%s\",%.4f,%d,%d,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",%d,%d,%d,%d,%d%n",
					++moveCounter,
					processId,
					sourceMachine,
					destinationMachine,
			...
			if (solutionTracker == null) {
				try {
					...
					solutionTracker.println("SolutionId,Cost,Improvement,Timestamp,SolverMethod,NumReassignments," +
						"LoadCost,BalanceCost,MachineMoveCost,ProcessMoveCost,ServiceMoveCost," +
						"NumConstraintsUnsatisfied,IsFeasible");
	...
	public static void trackSolutionStateChange(ImmutableSolution solution, String solverMethod) 
	{
		...
		synchronized (trackingLock) {
			if (solutionTracker != null) {
				...
				// Cast to SmartSolution to access additional metrics
				SmartSolution smartSol = (solution instanceof SmartSolution) ? 
					(SmartSolution) solution : new SmartSolution(solution);
				
				// Write solution state data with additional metrics
				solutionTracker.printf("%d,%d,%.4f,%d,\"%s\",%d,%d,%d,%d,%d,%d,%d,%s%n",
					currentSolutionId,
					solution.getCost(),
					...
					smartSol.getLoadCost(),
					smartSol.getBalanceCost(),
					smartSol.getMachineMoveCost(),
					smartSol.getProcessMoveCost(),
					smartSol.getServiceMoveCost(),
					smartSol.getNumConstraintsUnsatisfied(),
					smartSol.isFeasible()
				);
				solutionTracker.flush();
			}
