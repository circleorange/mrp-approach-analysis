...
double process_heuristic_weight_composite(usint const p){
	return (costOfProcess[pid_ds[p]]+process_requirement_cost[p])/(no_available_machines[pid_ds[p]]+0.1);
}
...
double service_heuristic_weight_composite(usint const i){
	...
	double cost = 0.0 ;
	for (j=0; j < service->no_of_unassigned_processes; j++)
		cost += (costOfProcess[pid_ds[service->unassigned_process[j]]]+process_requirement_cost[service->unassigned_process[j]])/(services[i].no_of_available_machines+1);
	return cost;
}
