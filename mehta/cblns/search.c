...
void save_solution(void) {
	...
	/* Track accepted reassignments by comparing previous_BM with current BM state */
	for (i = 0; i < no_of_unassigned_processes; i++) {
		usint process = unassigned_processes[i];
		usint new_machine = NM[process];
		usint old_machine = previous_BM[process];
	...
	/* Update previous_BM to track this accepted solution for next comparison */
...
void init_lns(void ){
	...
	simple_init_tracking("mehta_reassignments.csv");

	/* Initialize previous_BM with current best solution */
	{
		usint i;
		for (i = 0; i < no_processes; i++) {
			previous_BM[i] = BM[i];
...
void lns_machine_selection(int const local_time_limit ) {
	...
	do {
		...
		/* Store previous machine state before starting search */
		/* updatePreviousMachineState(); */ /* TODO: Implement for simple tracking */
		...
		/* Track reassignments after all processes have been assigned */
		/* trackCurrentReassignments(); */ /* TODO: Implement for simple tracking */
